...
// ============================================================================
// Three tiers — List, Contractor, Reseller — that exist only on the printed
// IPL copies, not in AutoCount. The app is the system of record, so these are
// stored here and edited in the app.
//
// Keyed on the part number with every separator stripped and upper-cased, the
// same normalised form the IPL viewer uses, so the IPL's "585 60 19-01" and
// AutoCount's "SZEN 585601901" both land on 585601901.
const normaliseCode = (code) =>
  String(code || "").replace(/[^A-Za-z0-9]/g, "").toUpperCase();

// A tier may legitimately be blank (not every part has all three). Blank stays
// null rather than becoming 0, so "no price recorded" never reads as "free".
...
// Prices are keyed on the IPL part number ("585 60 33-01" -> 585603301), but a
// caller may hand us the AutoCount code for the same part ("SZEN 585603301"),
// which keeps its brand prefix and so never matches exactly. Fall back to a
// suffix match, the same tolerance findItem() uses. The minimum length keeps a
// short code from matching half the table.
function findPriceRow(key) {
  const exact = db.prepare("SELECT * FROM part_prices WHERE item_code = ?").get(key);
  if (exact) return exact;
  if (key.length < 6) return null;
  return db.prepare(
    "SELECT * FROM part_prices WHERE ? LIKE '%' || item_code AND LENGTH(item_code) >= 6 ORDER BY LENGTH(item_code) DESC LIMIT 1"
  ).get(key) || null;
}

function getPartPrices(code) {
  const key = normaliseCode(code);
  if (!key) return null;
  return findPriceRow(key);
}

function setPartPrices(code, { list_price, contractor_price, reseller_price, updated_by = "" } = {}) {
  const key = normaliseCode(code);
  if (!key) { const e = new Error("A part number is required."); e.status = 400; throw e; }

  // Update the row this part already has rather than creating a second one
  // under a different spelling of the same code.
  const existing = findPriceRow(key);
  const storeKey = existing ? existing.item_code : key;

  const lp = cleanPrice(list_price);
  const cp = cleanPrice(contractor_price);
  const rp = cleanPrice(reseller_price);

  db.prepare(
    `INSERT INTO part_prices (item_code, list_price, contractor_price, reseller_price, updated_at, updated_by)
     VALUES (?, ?, ?, ?, datetime('now'), ?)
  ...
  ).run(storeKey, lp, cp, rp, String(updated_by || "").slice(0, 40));

  return getPartPrices(storeKey);
}

const partPrices = { getPartPrices, setPartPrices };
module.exports.partPrices = partPrices;
