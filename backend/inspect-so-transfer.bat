...
REM ===========================================================================
REM inspect-so-transfer.bat — run the read-only Sales Order transfer inspector on the server.
REM
REM The AutoCount credentials live in the OMService service configuration, not
REM in your command prompt, so running "node inspect-so-transfer.js" on its own fails
...
if not defined AUTOCOUNT_DB_SERVER (
  ...
  echo Set them by hand instead, then run: node inspect-so-transfer.js %1
