...
app.post("/api/slips", async (req, res) => {
  try {
    const { company, debtor_code, contact_name, contact_number, whatsapp_number, check_service, repair_only, quote_first, notes, machines, signature } = req.body || {};
    const slip = await data.slips.createSlip({ company, debtor_code, contact_name, contact_number, whatsapp_number, check_service, repair_only, quote_first, notes, machines, signature });
