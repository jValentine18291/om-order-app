...
console.log("\n-- the face that signs the quotation --");
// signature-font.js carries a subset of Great Vibes as base64, generated by
// the command in its own header. If that is ever regenerated wrongly - the
// wrong flags, a truncated copy, a data: prefix left on the front - jsPDF does
// not complain. It registers the font, draws nothing visible, and the
// quotation goes out with an empty line above "OUTBOARD & MARINE (PTE) LTD".
// So check the bytes really are a TrueType file, here, where it is cheap.
{
  const fs2 = require("fs");
  const src = fs2.readFileSync(
    path.resolve(__dirname, "..", "frontend", "signature-font.js"), "utf8");
  const m = src.match(/const SIG_B64 = "([A-Za-z0-9+/=]+)"/);
  check("the base64 is there and is only base64", !!m, true);
  const buf = m ? Buffer.from(m[1], "base64") : Buffer.alloc(0);
  // 00 01 00 00 is the sfnt version every TrueType file opens with.
  check("and decodes to a TrueType file",
    [...buf.slice(0, 4)], [0x00, 0x01, 0x00, 0x00]);
  check("of a size that says subset, not the whole 362 KB face",
    buf.length > 10000 && buf.length < 60000, true);
  // The name table has to still say what it is, or a reader shows a fallback.
  check("that still names itself Great Vibes",
    buf.includes(Buffer.from("Great Vibes", "utf16le")) ||
    buf.includes(Buffer.from("GreatVibes", "utf16le")) ||
    buf.includes(Buffer.from("Great Vibes", "ascii")), true);
}
